...
# oursim-trace-persistent_30_machines_7_dias_50_sites_5_upp_7.txt
setUpGName <- function(nSites, nRecursos, upp, dir, scheduler, rodada) {
	prefix <- paste( "oursim-trace-", scheduler, "_", sep="")
	sufix <- paste( "_sites_", upp, "_upp_", rodada, ".txt", sep="")
	setUpName(nSites, nRecursos, dir, prefix, sufix, scheduler, rodada)
}

# spot-trace-persistent_30_machines_7_dias_50_sites_10_upp_100_spotLimit_groupedbypeer_false_av_us-east-1.linux.m2.2xlarge.csv_10.txt
setUpCName <- function(nSites, nRecursos, upp, dir, scheduler, rodada, instance, spotLimit, groupedbypeer) {
	prefix <- paste( "spot-trace-", "persistent", "_", sep="" )
	sufix <- paste( "_sites_", upp, "_upp_", spotLimit,"_spotLimit_groupedbypeer_",groupedbypeer,"_av_",instance,"_", rodada, ".txt",sep="" )
	setUpName(nSites, refNumOfMachinesByPeer, dir, prefix, sufix, scheduler, rodada)
}

setUpGUName <- function(nSites, nRecursos, upp, dir, scheduler, rodada) {
	prefix <- paste( "oursim-trace-utilization-", scheduler, "_", sep="")
	sufix <- paste( "_sites_", upp, "_upp_", rodada, ".txt", sep="")
	setUpName(nSites, nRecursos, dir, prefix, sufix, scheduler, rodada)
}

setUpCUName <- function(nSites, nRecursos, upp, dir, scheduler, rodada, instance) {
	prefix <- paste( "spot-trace-utilization-", scheduler, "_", sep="")
	sufix <- paste( "_sites_", upp, "_upp_", "100_spotLimit_groupedbypeer_true_av_us-east-1.linux.m1.small.csv_", rodada, ".txt", sep="")
	setUpName(nSites, nRecursos, dir, prefix, sufix, scheduler, rodada)
}
...
generateRodadaID <- function(nPeers, nMachines, upp, instType, limit, rodada) {
	#npeers=[80]_nmacbypeer=[30]_upp=[5]_scheduler=persistent_spts=[m1.small]_limit=[100]_gbp=false_rodadas=[30]
	paste("npeers=[",nPeers,"]_nmacbypeer=[",nMachines,"]_upp=[",upp,"]_scheduler=persistent_spts=[",instType,"]_limit=[",limit,"]_gbp=false_rodadas=[",rodada,"]",sep="")
}
