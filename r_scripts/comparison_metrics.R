...
d_metric <- function (gw, cw) {
	g_metric <- d_grid_metric(gw)
	c_metric <- d_cloud_metric(cw)
...
d_metric_bp <- function (gw, cw, nPeers) {
	g_metric <- d_grid_metric(gw)
	c_metric <- d_cloud_metric(cw)
...
d_metric_bp2 <- function (gw, cw, nPeers) {
...
d_metric_2 <- function (gw, cw, type) {
	g_metric <- d_grid_metric(gw)
	c_metric <- d_cloud_metric(cw)
	metric <- NULL
	if (type=="mean") {
...
d_metric_mean <- function (gw, cw) {
	d_metric_2(gw,cw,"mean")
}
...
d_metric_mean <- function (nPeers, nMachinesByPeer, instance, rodada) {
	...
	d_metric_2(cw,gw,"mean")
}

d_metric_min <- function (gw, cw) {
	d_metric_2(gw,cw,"min")
}

d_metric_max <- function (gw, cw) {
	d_metric_2(gw,cw,"max")
}

extract_d_metric <- function (gfile, cfile, nPeers, nMachines, upp, instance, type, limit, rodada) {
